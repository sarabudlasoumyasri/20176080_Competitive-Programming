...
public class Solution {
    ...
    public static Rectangle findRectangularOverlap(Rectangle rect1, Rectangle rect2) {
        ...
        int res_leftX=Math.max(rect1.getLeftX(),rect2.getLeftX());
        int res_bottomY=Math.max(rect1.getBottomY(),rect2.getBottomY());
        int res_width=Math.min(rect1_rightX,rect2_rightX)-res_leftX;
        int res_height=Math.min(rect1_topY,rect2_topY)-res_bottomY;
        if(res_width<=0 || res_height<=0) return new Rectangle();
        else return new Rectangle(res_leftX,res_bottomY,res_width,res_height);
    }
